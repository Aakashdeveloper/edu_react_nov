...
class RoomFilter extends Component {
    filterRoom = (event) => {
        ...
        let tripId = sessionStorage.getItem('tripId')?sessionStorage.getItem('tripId'):1;
        let roomUrl;
        if(roomId == ""){
            roomUrl = `${url}/${tripId}`
        }else{
        ...
        return(
            <div onChange={this.filterRoom}>
                ...
                <div>
                    ...
                    <label className="radio">
                        <input type="radio" name="filter" value="2"/>Premiere Rooms
                    </label>
                    <label className="radio">
                        <input type="radio" name="filter" value="3"/>Travel
                    </label>
                    <label className="radio">
